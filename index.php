...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Debug mode: Check if ID is provided
    if ($debug && !empty(trim($_POST["id"]))) {
        $id = trim($_POST["id"]);
        ...
        $sql_id = "SELECT id, email, password FROM user WHERE id = ?";

        if ($stmt = mysqli_prepare($conn, $sql_id)) {
